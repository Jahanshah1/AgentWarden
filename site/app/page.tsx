...
export default function Home() {
  return <main>
    ...
    <section className="install-section" id="install"><div className="container install-layout"><div><p className="kicker">Get started</p><h2>Install once.<br />Keep your agent.</h2><p>AgentWarden is not a framework replacement. It is a local layer around the agent you already have.</p></div><div className="install-code"><header><span>Terminal</span><span>Python 3.11+</span></header><pre><code><span>$</span> pip install agentwarden{`\n`}<span>$</span> agentwarden serve{`\n`}{`\n`}# In your existing agent{`\n`}base_url = "http://127.0.0.1:8080/v1"</code></pre><a href={repoUrl} target="_blank" rel="noreferrer">Read the installation guide <ArrowRight size={16}/></a></div></div></section>
